...
if [ "$#" -ne 3 ]; then
  ...
  echo "Usage: "$0" <java_major_version> <java_minor_version> <java_build_version>"
	...
	if [ -f /vagrant/resources/installs/$JDK_RPM ]
		then
		echo "Java JDK $JDK_RPM exists, installing..."
	else
		echo "Java JDK $JDK_RPM not exists, downloading..."
		wget --no-check-certificate --no-cookies --header "Cookie: oraclelicense=accept-securebackup-cookie" -O /vagrant/$JDK_RPM $JDK_URL
	fi

	yum -y --nogpgcheck localinstall /vagrant/resources/installs/$JDK_RPM
